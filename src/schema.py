...
class AsyncStreamingResponse:
    ...
    async_response_gen: Any  # TokenAsyncGenの具体的な型に変更する
    source_nodes: List[Any] = field(default_factory=list)  # NodeWithScoreの具体的な型に変更する
    metadata: Optional[Dict[str, Any]] = None
    response_txt: Optional[str] = None
    ...
    def __str__(self) -> str:
        ...
        if loop.is_running():
            return self._sync_str()
        else:
            return loop.run_until_complete(self._async_str())

    def _sync_str(self) -> str:
        """Helper method to convert to string in a synchronous context."""
        future = asyncio.ensure_future(self._async_str())
        loop = asyncio.get_event_loop()
    ...
    async def _yield_response(self):
